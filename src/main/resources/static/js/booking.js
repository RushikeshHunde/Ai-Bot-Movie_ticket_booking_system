let movieTotals = {};
	...
	function init360(btn) {
	    const panoUrl = btn.getAttribute('data-pano');
	    const modal = document.getElementById('modal-360');
	    
	    // Show the modal
	    modal.classList.remove('hidden');

	    // Clean up existing viewer to avoid memory leaks
	    if (panoramaViewer) {
	...
	function showPOV(btn) {
	    const container = document.getElementById('pov-preview-container');
	    const img = document.getElementById('pov-preview-img');
	    const label = document.getElementById('pov-seat-label');

	    img.src = btn.getAttribute('data-preview');
	    label.innerText = "Seat " + btn.getAttribute('data-seat-num');
	    
	    container.classList.remove('hidden');
	    
	    btn.onmousemove = (e) => {
	        container.style.left = (e.clientX + 15) + 'px';
	        container.style.top = (e.clientY - 150) + 'px';
	    };
	}

	function hidePOV() {
	    document.getElementById('pov-preview-container').classList.add('hidden');
	}
