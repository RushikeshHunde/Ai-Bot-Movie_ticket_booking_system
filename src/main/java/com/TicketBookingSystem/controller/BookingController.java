...
import com.TicketBookingSystem.model.Theater;
...
public class BookingController {
	...
	public String proceedToCheckout(@RequestParam Long showId,
	    ...
	    if (showOpt.isEmpty()) {
	        return "redirect:/booking?error=invalid_show";
	    }
	...
	public String confirmPayment(@RequestParam Long bookingId, Model model) {
	    ...
	    ticket.setTheatreName(show.getTheatreName());
	    ticket.setCity(show.getTargetCity());
	...
	public String getSeatPov(@RequestParam String seatId, @RequestParam Long showId) {
	    Show show = showRepository.findById(showId).get();
	    Theater theater = show.getTheater(); // Get the linked theater

	    // Use the logic I gave you before, but pull URLs from theater.getPovFrontUrl()
	    return theater.getPovFrontUrl();
	}
